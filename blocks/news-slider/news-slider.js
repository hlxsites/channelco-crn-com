function addDragEvents(handle, carousel) {
    ...
    function moveDrag(e) {
        ...
        let newTransform = calculateTransform(currentTransform, dragShift);
    ...
    function endDrag(e) {
        if (dragStartedOnLink && isMinimalDrag(startMouseX, (e.changedTouches) ? e.changedTouches[0].clientX : e.clientX)) {
    ...
    function snapToBoundary(handle, boundary) {
        currentTransform = boundary;
        handle.style.transform = `translateX(${boundary}px)`;
    }
}
